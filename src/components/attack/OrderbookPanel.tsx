...
export function OrderbookPanel({ r }: { r: SweepResult }) {
  return (
    <section className="border border-line p-4 font-mono text-sm">
      <h2 className="mb-2 text-xs text-muted">{"// THIN ORDERBOOK · LAST-TRADE ORACLE"}</h2>
      ...
      <p className="mt-4 text-xs text-muted">22 Feb 2026: one trade on a market with almost no volume moved USTRY from ~$1 to ~$107. YieldBlox accepted that print as collateral value.</p>
    </section>
  );
}
