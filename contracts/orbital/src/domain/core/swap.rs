...
use crate::{errors::OrbitalError, Result};
...
pub fn compute_fee(amount_in: FixedPoint, fee_rate_bps: u32) -> Result<FixedPoint> {
    if fee_rate_bps == 0 {
...
pub fn compute_slippage_bps(mid_price: FixedPoint, execution_price: FixedPoint) -> Result<u16> {
...
pub fn execute_swap(
    ...
    let old_in_reserve = pool.reserves.get(token_in).unwrap();
    let old_out_reserve = pool.reserves.get(token_out).unwrap();
...
mod tests {
    ...
    fn test_swap_happy_path() {
        ...
        assert!(result
            .amount_in
            .approx_eq(amount_in, FixedPoint::from_int(1)));
