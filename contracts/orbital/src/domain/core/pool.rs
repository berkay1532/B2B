...
use crate::math::{fixed_point::FixedPoint, reserve_state::ReserveState, sphere::Sphere};
...
fn build_reserve_state(pool: &PoolState) -> Result<ReserveState> {
    ReserveState::new(pool.reserves.clone(), pool.n_assets)
}
...
pub fn compute_radius_from_deposit(deposit: FixedPoint, n: u32) -> Result<FixedPoint> {
...
pub fn compute_radius_from_reserves(reserves: Vec<FixedPoint>, n: u32) -> Result<FixedPoint> {
    require!(
        n >= 2 && (n as usize) <= MAX_ASSETS,
        OrbitalError::InvalidAssetCount
    );
    ...
    for i in 0..n {
        require!(
            reserves.get(i).unwrap().raw >= 0,
            OrbitalError::InvalidLiquidityAmount
        );
        sum_x = sum_x.checked_add(reserves.get_unchecked(i))?;
        sum_x_sq = sum_x_sq.checked_add(reserves.get_unchecked(i).squared()?)?;
    }
...
pub fn recompute_sphere(pool: &mut PoolState) -> Result<FixedPoint> {
    let new_radius = compute_radius_from_reserves(pool.reserves.clone(), pool.n_assets)?;
    pool.sphere = Sphere {
        radius: new_radius,
        n: pool.n_assets,
    };
...
pub fn derive_vault_pda(
    ...
    //     program_id,
    // )
    todo!("not relevant for soroban, redesign")
}
...
pub fn initialize_pool_reserves(
    ...
    token_mints: &Vec<Address>,
    token_vaults: &Vec<Address>,
) -> Result<()> {
    let n = pool.n_assets;

    // 1. Validate inputs
    require!(
        token_mints.len() == n && token_vaults.len() == n,
        OrbitalError::InvalidAssetCount
    );
    require!(
    ...
    for i in 0..n {
        // require!(
        //     token_mints.get_unchecked(i) != Address::default(),
        //     OrbitalError::InvalidTokenIndex
        // );
        for j in (i + 1)..n {
            require!(
                token_mints.get_unchecked(i) != token_mints.get_unchecked(j),
    ...
    for i in 0..n {
        pool.reserves.set(i, per_asset_deposit);
    }
    for i in n..(MAX_ASSETS as u32) {
        pool.reserves.set(i, FixedPoint::zero());
    }

    // 4. Store token references
    for i in 0..n {
        pool.token_mints.set(i, token_mints.get_unchecked(i));
        pool.token_vaults.set(i, token_vaults.get_unchecked(i));
    }
...
mod tests {
    ...
    use super::*;
    use crate::domain::core::test_helpers::{init_pool, make_pool, sqrt_epsilon, unique_pubkeys};
    ...
    fn test_verify_invariant_fails_broken_state() {
        ...
        pool.reserves.set(0, FixedPoint::from_int(999));
    ...
    fn test_verify_invariant_o1_matches_on() {
        ...
        let on_result = pool
            .sphere
            .check_invariant(&pool.reserves.slice(..pool.n_assets));
    ...
    fn test_update_caches_equal_reserves() {
        ...
        assert!(
            pool.alpha_cache
                .approx_eq(expected_alpha, FixedPoint::from_int(1)),
            "alpha ({:?}) should ≈ 173",
            pool.alpha_cache
        );
    ...
    fn test_update_caches_unequal_reserves() {
        ...
        pool.reserves.set(0, FixedPoint::from_int(80));
        pool.reserves.set(1, FixedPoint::from_int(120));
        update_caches(&mut pool).unwrap();

        let rs = ReserveState::new(pool.reserves.clone(), pool.n_assets).unwrap();
    ...
    fn test_initialize_pool_reserves_n3_equal() {
        ...
        for i in 0..3 {
            assert_eq!(pool.reserves.get_unchecked(i).raw, deposit.raw);
        }
        // Inactive reserves should be zero
        for i in 3..MAX_ASSETS {
            assert_eq!(pool.reserves.get_unchecked(i).raw, 0);
        }
    ...
    fn test_initialize_pool_reserves_stores_mints_and_vaults() {
        ...
        for i in 0..3 {
            assert_eq!(pool.token_mints.get_unchecked(i), mints.get_unchecked(i));
            assert_eq!(pool.token_vaults.get_unchecked(i), vaults.get_unchecked(i));
        }
    }
    ...
    fn test_initialize_pool_reserves_rejects_duplicate_mints() {
        ...
        let mints = vec![&Env::default(), mint_a, mint_b, mint_a]; // Duplicate mint_a
        let vaults = unique_pubkeys(3);
        assert!(initialize_pool_reserves(&mut pool, deposit, &mints, &vaults).is_err());
    }
    ...
    fn test_initialize_pool_reserves_rejects_zero_mint() {
        ...
        let mint_a = Address::new_unique();
        let mint_b = Address::new_unique();
        let mints = vec![mint_a, Address::default(), mint_b]; // Zero key
        let vaults = unique_pubkeys(3);
        assert!(initialize_pool_reserves(&mut pool, deposit, &mints, &vaults).is_err());
    }
    ...
    fn test_compute_radius_from_reserves_matches_equal_deposit() {
        ...
        for n in [2u8, 3, 5, 8] {
            ...
            for i in 0..(n as usize) {
                reserves.get_unchecked(i) = deposit;
            }
