...
use crate::{
    domain::core::{recompute_sphere, update_caches, verify_invariant},
    errors::OrbitalError,
...
pub struct RemoveLiquidityResult {
    ...
    pub return_amounts: Vec<FixedPoint>,
    /// Per-token amounts as u64 (for SPL transfers)
    pub return_amounts_u64: Vec<u64>,
...
pub fn add_liquidity_to_pool(
    ...
    for i in 0..n {
        require!(
            deposits.get_unchecked(i).is_positive(),
    ...
    for i in 0..n {
        pool.reserves.set(
            ...
            pool.reserves
                .get_unchecked(i)
                .checked_add(deposits.get_unchecked(i))?,
        );
        liquidity = liquidity.checked_add(deposits.get_unchecked(i))?;
    }
...
mod tests {
    ...
    fn test_remove_liquidity_floor_prevents_overshoot() {
        ...
        for i in 0..3 {
            let reconverted =
                FixedPoint::from_token_amount(result.return_amounts_u64[i], pool.token_decimals[i])
                    .unwrap();
            assert!(
