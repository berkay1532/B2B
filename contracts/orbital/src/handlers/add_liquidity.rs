...
use crate::state::pool::{read_pool, PoolState, POOL};
...
pub fn handler<'info>(env: Env, params: AddLiquidityParams) -> Result<()> {
    ...
    for i in 0..n {
        require!(
            params.amounts.get_unchecked(i) > 0,
    ...
    for i in 0..n {
        require!(
            *remaining.get_unchecked(i).key == pool.token_vaults.get_unchecked(i),
    ...
    for i in 0..n {
        ...
        let vault_info = &remaining.get_unchecked(i);

        let token = token::Client::new(&env, todo!());

        let res: () = token.transfer(
            ata_info.clone(),
            vault_info.clone(),
            params.amounts.get_unchecked(i),
        );
    }
    ...
    for i in 0..n {
        deposits_fp.get_unchecked(i) = FixedPoint::from_token_amount(
            params.amounts.get_unchecked(i),
            pool.token_decimals.get_unchecked(i),
        )?;
    }
    ...
    if has_tick {
        ...
        for i in 0..n {
            tick.reserves.get_unchecked(i) = tick
                .reserves
                .get_unchecked(i)
                .checked_add(deposits_fp.get_unchecked(i))?;
        }
        ...
        if tick.status == TickStatus::Boundary {
            for i in 0..n {
                pool.reserves.get_unchecked(i) = pool
                    .reserves
                    .get_unchecked(i)
                    .checked_sub(deposits_fp.get_unchecked(i))?;
            }
