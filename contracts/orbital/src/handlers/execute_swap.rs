...
use crate::state::pool::read_pool;
use crate::state::{pool::PoolState, tick::TickStatus};
...
pub fn handler<'info>(env: Env, params: SwapParams) -> Result<()> {
    ...
    token.transfer(user_ata_in_info, vault_in_info, params.amount_in);
    ...
    let (total_out, total_fee) = if tick_accounts.is_empty() {
        ...
        while remaining_in.raw > 0 && iteration < max_iterations {
            ...
            let tick_boundaries = find_nearest_tick_boundaries(&tick_data_pairs, pool.alpha_cache);
            ...
            let tentative_alpha = compute_new_alpha(running_sum, remaining_in, tentative_out, n)?;

            // 5. Determine if a tick crossing occurs
            let crossing_k =
                determine_crossing_k(pool.alpha_cache, tentative_alpha, &tick_boundaries);
        ...
        require!(remaining_in.raw <= 0, OrbitalError::TickCrossingFailed);
    ...
    let amount_out_u64 = total_out.to_token_amount_floor(pool.token_decimals[token_out])?;
        ...
        if tick.k.raw == k_cross.raw {
            ...
            match tick.status {
                TickStatus::Interior => {
                    ...
                    pool.total_interior_liquidity =
                        pool.total_interior_liquidity.checked_sub(tick.liquidity)?;
                    pool.total_boundary_liquidity =
                        pool.total_boundary_liquidity.checked_add(tick.liquidity)?;
                }
                TickStatus::Boundary => {
                    ...
                    pool.total_boundary_liquidity =
                        pool.total_boundary_liquidity.checked_sub(tick.liquidity)?;
                    pool.total_interior_liquidity =
                        pool.total_interior_liquidity.checked_add(tick.liquidity)?;
                }
            }
