...
impl FixedPoint {
    ...
    pub fn from_token_amount(raw: u64, decimals: u8) -> Result<Self> {
        ...
        if result > i128::MAX as u128 {
            return Err(crate::errors::OrbitalError::MathOverflow);
        }
        Ok(Self {
            raw: result as i128,
        })
    ...
    pub fn checked_div(self, rhs: Self) -> Result<Self> {
        require!(rhs.raw != 0, crate::errors::OrbitalError::DivisionByZero);
    ...
    pub fn sqrt(self) -> Result<Self> {
        require!(self.raw >= 0, crate::errors::OrbitalError::SqrtNegative);
...
mod tests {
    ...
    fn test_div_large_values() {
        ...
        assert!(
            c.raw >= expected_lo.raw && c.raw <= expected_hi.raw,
            "1000000/3 should be ~333333, got {:?}",
            c
        );
    }
    ...
    fn test_div_fractional_result() {
        ...
        let epsilon = FixedPoint::from_raw(2); // minimal tolerance
        assert!(
            c.approx_eq(third_approx, epsilon),
            "1/3 should be ~0.333, got {:?}",
            c
        );
    }
    ...
    fn test_from_token_amount_sub_token() {
        ...
        assert!(
            fp.raw > 0,
            "sub-token amount should produce positive FixedPoint"
        );
    ...
    fn test_roundtrip_9_decimals() {
        ...
        let test_values: &[u64] = &[0, 1_000_000_000, 1_500_000_000, 999_999_999];
        for &val in test_values {
    ...
    fn test_floor_exact_values() {
        ...
        assert_eq!(
            fp.to_token_amount(6).unwrap(),
            fp.to_token_amount_floor(6).unwrap()
        );

        let fp2 = FixedPoint::from_fraction(3, 2).unwrap(); // 1.5
        assert_eq!(
            fp2.to_token_amount(6).unwrap(),
            fp2.to_token_amount_floor(6).unwrap()
        );
    }
