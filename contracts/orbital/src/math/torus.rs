...
impl TorusParams {
    ...
    pub fn from_pool_liquidity(total_interior: FixedPoint, total_boundary: FixedPoint) -> Self {
        Self {
...
use crate::state::tick::TickStatus;
...
pub fn find_nearest_tick_boundaries(
    ticks: Vec<(FixedPoint, TickStatus)>,
...
pub fn compute_delta_to_boundary(
    ...
    for i in 0..n as usize {
        running_sum = running_sum.checked_add(reserves.get_unchecked(i))?;
    }
    ...
    let discriminant = b_coeff
        .squared()?
        .checked_sub(FixedPoint::from_int(4).checked_mul(c_coeff)?)?;
    ...
    let result = match (root1.raw > 0, root2.raw > 0) {
        ...
        (false, false) => {
            ...
            // msg!(
            //     "compute_delta_to_boundary: no positive root for k_cross={}, roots=({}, {})",
            //     k_cross.raw,
            //     root1.raw,
            //     root2.raw
            // );
...
mod tests {
    ...
    fn make_sphere(r: i64, n: u32) -> Sphere {
        Sphere {
    ...
    fn test_torus_params_has_boundary_liquidity() {
        let tp =
            TorusParams::from_pool_liquidity(FixedPoint::from_int(100), FixedPoint::from_int(50));
        assert!(!tp.is_single_sphere());
        assert!(tp.has_boundary_liquidity());
    }
    ...
    fn test_torus_params_interior_only() {
        let tp = TorusParams::from_pool_liquidity(FixedPoint::from_int(100), FixedPoint::zero());
        assert!(!tp.is_single_sphere());
        assert!(!tp.has_boundary_liquidity());
    }
    ...
    fn test_orthogonal_radius_at_equal_price_zero() {
        ...
        let alpha_eq = sphere
            .radius
            .checked_mul(sqrt_n.checked_sub(one).unwrap())
            .unwrap();
        let s = orthogonal_radius(&sphere, alpha_eq).unwrap();
    ...
    fn test_orthogonal_radius_rejects_large_negative_radicand() {
        ...
        assert!(
            result.is_err(),
            "Large negative radicand should be rejected"
        );
    }
    ...
    fn test_compute_new_alpha_net_inflow() {
        ...
        let new_alpha =
            compute_new_alpha(sum, FixedPoint::from_int(20), FixedPoint::from_int(10), 3).unwrap();
    ...
    fn test_find_nearest_empty_ticks() {
        let data =
            find_nearest_tick_boundaries(Vec::new(&Env::default()), FixedPoint::from_int(100));
    ...
    fn test_find_nearest_mixed_ticks() {
        ...
        assert_eq!(
            data.nearest_k_lower.unwrap().raw,
            FixedPoint::from_int(95).raw
        );
        assert_eq!(
            data.nearest_k_upper.unwrap().raw,
            FixedPoint::from_int(105).raw
        );
    }
    ...
    fn test_delta_zero_root_returns_zero_not_positive() {
        ...
        assert_eq!(
            delta2.raw, 0,
            "zero root must be returned for 1→0, got {}",
            delta2
        );
    }
