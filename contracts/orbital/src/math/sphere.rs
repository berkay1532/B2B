...
impl Sphere {
    ...
    pub fn new(total_liquidity: FixedPoint, n: u32) -> Result<Self> {
        require!(
    ...
    pub fn distance_squared(&self, reserves: &Vec<FixedPoint>) -> Result<FixedPoint> {
        require!(
    ...
    pub fn price(&self, i: u32, j: u32, reserves: Vec<FixedPoint>) -> Result<FixedPoint> {
        ...
        let numerator = self.radius.checked_sub(reserves.get_unchecked(j))?;
        let denominator = self.radius.checked_sub(reserves.get_unchecked(i))?;
    ...
    pub fn check_invariant(&self, reserves: &Vec<FixedPoint>) -> Result<()> {
...
mod tests {
    ...
    fn test_sphere_price_equal_reserves() {
        ...
        let reserves = vec![&Env::default(), q, q, q];
