...
use crate::{
    math::{fixed_point::FixedPoint, sphere::Sphere},
    require,
    state::tick::TickStatus,
    Result,
};
...
impl Tick {
    ...
    pub fn k_min(sphere: &Sphere) -> Result<FixedPoint> {
        ...
        sphere
            .radius
            .checked_mul(sqrt_n.checked_sub(FixedPoint::one())?)
    }
    ...
    #[inline(never)]
    fn compute_capital_efficiency(sphere: &Sphere, x_min: FixedPoint) -> Result<FixedPoint> {
...
mod tests {
    ...
    fn test_k_at_k_min_rejected() {
        ...
        let k_min = Tick::k_min(&sphere).unwrap();
        assert!(
            Tick::new(k_min, &sphere).is_err(),
            "k == k_min should be rejected (strict inequality)"
        );
    }
    ...
    fn test_k_at_k_max_rejected() {
        ...
        let k_max = Tick::k_max(&sphere).unwrap();
        assert!(
            Tick::new(k_max, &sphere).is_err(),
            "k == k_max should be rejected (strict inequality)"
        );
    }
