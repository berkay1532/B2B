...
impl ReserveState {
    ...
    pub fn active_amounts(&self) -> Vec<FixedPoint> {
        self.amounts.slice(..self.n)
    }
    ...
    pub fn apply_trade(
        ...
        let new_running_sum = self
            .running_sum
            .checked_add(amount_in)?
            .checked_sub(amount_out)?;

        let new_running_sq_sum = self
            .running_sq_sum
...
mod tests {
    ...
    fn test_price_unequal_reserves() {
        ...
        let sphere = Sphere {
            radius: FixedPoint::from_int(200),
            n: 2,
        };
