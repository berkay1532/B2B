...
fn validate_swap_inputs(
    ...
    require!(
        reserves.get_unchecked(token_out).raw > 0,
...
pub fn compute_amount_out_analytical(
    ...
    let r = sphere.radius;
    let a = r.checked_sub(reserves.get_unchecked(token_in))?; // r - x_in
    let b = r.checked_sub(reserves.get_unchecked(token_out))?; // r - x_out
    let d = net_amount_in;
...
fn bisection_solve(
    ...
    if (f_lo.raw > 0) == (f_hi.raw > 0) {
        return Err(crate::errors::OrbitalError::SolverDidNotConverge);
    }
    ...
    for _ in 0..bisect_iters {
        let mid_raw = lo
            .raw
            .checked_add(hi.raw)
            .ok_or_else(|| crate::errors::OrbitalError::MathOverflow)?
            / 2;
    ...
    Err(crate::errors::OrbitalError::SolverDidNotConverge)
}
...
mod tests {
    ...
    fn make_equal_reserves(env: Env, total_liquidity: i64, n: u32) -> (Sphere, Vec<FixedPoint>) {
    ...
    fn make_imbalanced_reserves(
        ...
        n: u32,
        imbalance: i64,
    ) -> (Sphere, Vec<FixedPoint>) {
        let sphere = Sphere::new(FixedPoint::from_int(total_liquidity), n).unwrap();
        let q = sphere.equal_price_point().unwrap();
        let mut reserves = Vec::new(env);
        for i in 0..(MAX_ASSETS as u32) {
            reserves.push_back(FixedPoint::zero())
        }
        for i in 0..n as usize {
            reserves[i] = q;
        }
        ...
        let s_rest = c_sq
            .checked_mul(FixedPoint::from_int((n - 2) as i64))
            .unwrap();
    ...
    fn test_analytical_equal_reserves_n3() {
        ...
        let d_out = compute_amount_out_analytical(&sphere, &reserves, 0, 1, d_in).unwrap();
    ...
    fn test_analytical_matches_legacy_n3_helper() {
        ...
        let two_cd = c
            .checked_mul(d_in)
            .unwrap()
            .checked_mul(FixedPoint::from_int(2))
            .unwrap();
        ...
        let new_out = compute_amount_out_analytical(&sphere, &reserves, 0, 1, d_in).unwrap();
    ...
    fn test_analytical_same_token_error() {
        let (sphere, reserves) = make_equal_reserves(3_000, 3);
        let d_in = FixedPoint::from_int(10);
        assert!(compute_amount_out_analytical(&sphere, &reserves, 0, 0, d_in,).is_err());
    }
    ...
    fn test_analytical_zero_amount_error() {
        let (sphere, reserves) = make_equal_reserves(3_000, 3);
        assert!(
            compute_amount_out_analytical(&sphere, &reserves, 0, 1, FixedPoint::zero(),).is_err()
        );
    }
    ...
    fn test_analytical_too_large_trade_error() {
        ...
        assert!(compute_amount_out_analytical(&sphere, &reserves, 0, 1, huge,).is_err());
    }
    ...
    fn test_newton_matches_analytical_n3() {
        ...
        let analytical = compute_amount_out_analytical(&sphere, &reserves, 0, 1, d_in).unwrap();

        let newton = solver.solve(&sphere, &reserves, 0, 1, d_in).unwrap();
    ...
    fn test_newton_converges_within_10_iterations() {
        ...
        assert!(
            result.is_ok(),
            "Newton should converge within 10 iterations"
        );
        assert!(verify_invariant_after_swap(
            &sphere,
            &reserves,
    ...
    fn test_analytical_zero_reserve_out_error() {
        ...
        assert!(compute_amount_out_analytical(&sphere, &reserves, 0, 1, d_in,).is_err());
    }
    ...
    fn test_newton_clamp_does_not_false_converge() {
        ...
        let d_in = x_out
            .checked_mul(FixedPoint::from_fraction(9, 10).unwrap())
            .unwrap();
        let solver = NewtonSolver::default_solver();
